...
// description, and a best-fit category. Best-effort — any field it can't read
// comes back null and the person just fills it in.

const { ok, error, methodGuard, parseBody } = require('./lib/http');
const { verifyRequest } = require('./lib/google');
const { CURRENCY_CODES, CATEGORY_NAMES } = require('./lib/domain');
...
const RECEIPT_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      category: { type: ['string', 'null'], enum: [...CATEGORY_NAMES, null], description: 'The single best-fit expense category from the allowed list.' },
    },
...
const PROMPT =
  ...
  'the description to English. Pick the single best-fit category. If a field is not legible, ' +
...
// Keep only values the app can actually use; drop anything off-list.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    return ok({ scan: normalize(toolUse ? toolUse.input : {}) });
  } catch (err) {
